...
액터
  ...
  이는 다국어 관리처럼 사용자가 추가/제거할 수 있어야 한다.
...
스크립트
  ...
  모든 텍스트 입력은 직접 입력하는 방식으로 해결하지만, 대신 옆에서 다국어를 추가/지정할 수 있다.
...
다국어 지원
  ...
  다국어 대본을 따로 JSON 형식 파일로 관리하며, 다국어의 입력이 필요한 부분에 대본 파일에 기입된 UUID값이 입력된다.
  예를 들어 한국어와 영어를 사용 중이라면, 다음과 같다.


  한국어.json

  ... {
    "521d3773-fcd1-4451-9f7f-ce0cf727e244": "한국어가 입력되어 있습니다."
  }

  영어.json

  ... {
    "521d3773-fcd1-4451-9f7f-ce0cf727e244": "The English part."
  }


  이는 빌드시 하나의 대본파일로 병합되며 (Scenario.json), uuid 값을 통해 대본파일에서 동적으로 텍스트를 불러온다.
...
변수 입력
  ...
  변수값 (value)
    : 다국어를 지원하지 않습니다. 숫자를 포함한 것들이 이쪽에 해당됩니다.

  파일 (file)
    : 에셋에 있는 파일 목록을 보여줍니다. list 속성과 함께 사용해야하며, list 속성값은 배열로써, 가져올 파일 목록의 확장자를 담습니다.
      예를 들어 list: ['.mp3', '.mp4'] 는 mp3, mp4 파일만을 보여줍니다.

  선택 (select)
    : 버튼 목록을 보여줍니다. list 속성과 함께 사용해야하며, list 속성값은 배열로써, 버튼의 정보를 담습니다.
      예를 들어 list: [{ text: '1번', value: 1 }, { text: '2번', value: 2 }] 와 같이 사용할 수 있습니다.
...
스크립트 매크로 변수 종류
  ...
  값(value)
    : 다국어와 관련없이 전세계에서 통용되는 문자를 표현합니다. (숫자 등)
    : 변수를 입력할 수 있어야 합니다.
  ...
  선택(select)
    : 여러가지 선택지 중 한개만을 선택할 수 있어야 합니다.
    : 필터로 목록을 얻을 수 있어야 합니다.
...
액터
  ...
  : 물리효과를 받을 수 있어야 합니다.
  : 엔진 상에서 화면에 직접 배치할 수 있어야 합니다. 벽, 땅 등의 역할 때문입니다.
  : 설계 단계에서 머리, 배, 다리 등 원하는 부분을 지정할 수 있어야 합니다. 이후 스크립트에서 특정 부분 확대 기능을 위해서입니다.
  : 크기, 이름 등 객체마다 다를 수 있는 값을 스크립트에서 넘겨받을 수 있어야 합니다.


인터페이스

  : 물리효과에 적용받지 않습니다.
  : 엔진 상에서 화면에 직접 배치할 수 있어야 합니다. 스크립트로 배치할 순 없지만, 보이기/숨기기 정도는 가능해야 합니다.
